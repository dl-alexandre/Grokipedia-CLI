...
func (c *UpdateCheckCmd) Run(globals *Globals) error {
	...
	if !c.Force && globals.appCache != nil {
		if cached, ok := globals.appCache.Get(cacheKey); ok {
			var info UpdateInfo
	...
	// Cache the result
	if globals.appCache != nil {
		if data, err := json.Marshal(info); err == nil {
			_ = globals.appCache.Set(cacheKey, data)
		}
	}
...
func buildGitHubAPIURL() string {
	...
	return fmt.Sprintf("https://api.github.com/repos/grokipedia/%s/releases/latest", repo)
}
...
func AutoUpdateCheck(appCache *cache.Cache) {
	...
	if appCache != nil {
		if cached, ok := appCache.Get(cacheKey); ok {
			var info UpdateInfo
			if err := json.Unmarshal(cached, &info); err == nil && !info.UpdateAvailable {
				return
			}
		}
	}
